...
#include "planner.h"
      ...
      #define SCARA_CROSSTALK_FACTOR 0 // No crosstalk on original MPSCARA because shoulder and elbow have equal reduction ratio
    #endif
...
float segments_per_second = TERN(AXEL_TPARA, TPARA_SEGMENTS_PER_SECOND, SCARA_SEGMENTS_PER_SECOND);
  ...
  void forward_kinematics(const_float_t a, const_float_t b) {
    ...
    SERIAL_ECHOLNPGM("  OFFSET_FK_XY (x,y) ", cartes.x, ",", cartes.y);
    SERIAL_ECHOLNPGM("  FK_XY (x,y) ", (cartes.x - a_cos - b_cos), ",", (cartes.y - a_sin - b_sin));
    */
...
#elif ENABLED(MP_SCARA) // HOMING PROCESS FOR THE MP_SCARA
  ...
  void scara_set_axis_is_at_home(const AxisEnum axis) {
    ...
    // So the triangle uses points the S(shoulder), E(elbow) and ee(end effector)
    // so the "law of cosines" can be used to calculate the shoulder and elbow angles.
    // Theta1 is angle from absolute x-axis to link L1
    // Theta2 is the angle between link L1 and link L2
    // hand is introduced to be able to switch between a RHS or LHS system, in configuration.h you can find the variable too
    // SCARA_IS_RIGHT_HANDED = 1 is RHS, and = -1 is LHS
    // ALFA = angle between L1 and EE
  ...
  void inverse_kinematics(const xyz_pos_t &raw) {
    ...
    delta.set(DEGREES(THETA1), DEGREES(THETA2), raw.z);
      ...
      SERIAL_ECHOLNPGM("SCARA IK", raw);
      SERIAL_ECHOLNPGM("SCARA IK", delta);
      SERIAL_ECHOLNPGM("  SCARA (x,y) ", x, ",", y," Theta1=", DEGREES(THETA1), " Theta2=", DEGREES(THETA2));
